...
import { planFeatures, OPTIONS, kycStatus, verificationLevel, ROLES, SERVICES, confirmMessages } from 'src/app/helpers';
...
@Component({
	...
	styleUrls: ['./voter-id.component.scss'],
})
export class VoterIdComponent implements OnInit {
	...
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.VOTER_ID),
	...
	create(): void {
		...
		this.kycService.create(this.dataForm.getRawValue(), url).subscribe({
			next: (result) => {
				...
				this.updateCardValidator(false);
				this.spinnerService.stop();
				this.toastService.success('Voter Id added successfully');
				this.dataForm.patchValue(result.data);
				this.form['additionalDetails'].setValue(JSON.parse(result.data.additionalDetails));
				this.verificationDetailsService.updateCheckList('voter-id', null);
				this.verificationDetailsService.showCompleteModal('voter-id', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	update(): void {
		...
		this.kycService.update(this.dataForm.value, url).subscribe({
			next: (result) => {
				...
				this.updateCardValidator(false);
				this.spinnerService.stop();
				this.toastService.success('Voter Id details update successfully');
				this.dataForm.patchValue(result.data);
				this.form['additionalDetails'].setValue(JSON.parse(result.data.additionalDetails));
			},
			error: (error) => {
	...
	patch(payload) {
		this.kycService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('voter-id', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'voter-id', this.userData);
	}
