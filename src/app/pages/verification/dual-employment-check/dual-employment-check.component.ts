...
@Component({
	selector: 'app-dual-employment-check',
	templateUrl: './dual-employment-check.component.html',
	styleUrls: ['./dual-employment-check.component.scss'],
})
export class DualEmploymentCheckComponent implements OnInit {
	userData: any = {};
	errorMessages = kycFieldForm;
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.DUAL_EMPLOYMENT),
		cardNumber: new FormControl('', [Validators.required, Validators.pattern(OPTIONS.uanCardPattern)]),
		...
		this.kycService.getType(payload).subscribe({
			next: (result) => {
				...
				if (result.data) {
					...
					this.dualEmployment = result.data;
				...
				if ([ROLES.EMPLOYEE].includes(this.currentUser.role)) {
					this.form['vendorId'].clearValidators();
	...
	create(): void {
		...
		this.kycService.create(this.dataForm.getRawValue(), url).subscribe({
			next: (result) => {
				...
				this.updateCardValidator(false);
				this.spinnerService.stop();
				this.toastService.success('dual employment details added successfully');
				this.dataForm.patchValue(result.data);
				this.form['additionalDetails'].setValue(JSON.parse(result.data.additionalDetails));
				this.verificationDetailsService.showCompleteModal('dual-employment-check', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	update(): void {
		...
		this.kycService.update(this.dataForm.value, url).subscribe({
			next: (result) => {
				...
				this.updateCardValidator(false);
				this.spinnerService.stop();
				this.toastService.success('dual employment details update successfully');
				this.dataForm.patchValue(result.data);
				this.form['additionalDetails'].setValue(JSON.parse(result.data.additionalDetails));
			},
			error: (error) => {
	...
	patch(payload) {
		this.kycService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('dual-employment-check', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'dual-employment-check', this.userData);
	}
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.DUAL_EMPLOYMENT)
		);
	}
}
