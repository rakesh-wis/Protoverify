...
@Component({
	...
	templateUrl: './gap-check.component.html',
	styleUrls: ['./gap-check.component.scss'],
})
export class GapCheckComponent implements OnInit {
	userData: any = {};
	errorMessages = kycFieldForm;
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.GAP_CHECK),
	...
	create(): void {
		...
		this.kycService.create(this.dataForm.getRawValue(), url).subscribe({
			next: (result) => {
				...
				this.updateCardValidator(false);
				this.spinnerService.stop();
				this.toastService.success('gap check details added successfully');
				this.dataForm.patchValue(result.data);
				this.form['additionalDetails'].setValue(JSON.parse(result.data.additionalDetails));
				this.verificationDetailsService.updateCheckList('gap-check', null);
				this.verificationDetailsService.showCompleteModal('gap-check', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	update(): void {
		...
		this.kycService.update(this.dataForm.value, url).subscribe({
			next: (result) => {
				...
				this.updateCardValidator(false);
				this.spinnerService.stop();
				this.toastService.success('gap check details update successfully');
				this.dataForm.patchValue(result.data);
				this.form['additionalDetails'].setValue(JSON.parse(result.data.additionalDetails));
			},
			error: (error) => {
	...
	patch(payload) {
		this.kycService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('gap-check', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'gap-check', this.userData);
	}
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.GAP_CHECK)
		);
	}
}
