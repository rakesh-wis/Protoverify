...
@Component({
	selector: 'app-ofac-check',
	templateUrl: './ofac-check.component.html',
	styleUrls: ['./ofac-check.component.scss'],
})
export class OfacCheckComponent implements OnInit {
	userData: any = {};
	errorMessages = businessDocumentsFieldForm;
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.OFAC_CHECK),
	...
	onSubmit(): void {
		...
		this.verificationMediaService.createBulk(this.dataForm.getRawValue()).subscribe({
			next: (result) => {
				this.spinnerService.stop();
				this.ofacCheck = result.data;
				this.dataForm.patchValue(result.data);
				this.toastService.success(result.message);
				this.verificationDetailsService.showCompleteModal('ofac-check', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	patch(payload) {
		this.verificationDetailsService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('ofac-check', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'cibil-check', this.userData);
	}
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.CIBIL)
		);
	}
}
