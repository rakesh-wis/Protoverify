...
export class CibilCheckComponent implements OnInit {
	...
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.CIBIL),
	...
	onSubmit(): void {
		...
		this.verificationMediaService.createBulk(this.dataForm.getRawValue()).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.showCompleteModal('cibil-check', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	patch(payload) {
		this.verificationDetailsService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('cibil-check', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
