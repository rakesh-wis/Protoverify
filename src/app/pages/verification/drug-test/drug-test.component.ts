...
@Component({
	...
	styleUrls: ['./drug-test.component.scss'],
})
export class DrugTestComponent implements OnInit {
	...
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.DRUG_TEST),
	...
	onSubmit(): void {
		...
		this.verificationMediaService.createBulk(this.dataForm.getRawValue()).subscribe({
			next: (result) => {
				this.spinnerService.stop();
				this.drugTest = result.data;
				this.dataForm.patchValue(result.data);
				this.toastService.success(result.message);
				this.verificationDetailsService.showCompleteModal('drug-test', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	patch(payload) {
		this.verificationDetailsService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('drug-test', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'drug-test', this.userData);
	}
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.DRUG_TEST)
		);
	}
}
