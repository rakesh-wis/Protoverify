...
@Component({
	selector: 'app-directorship-test',
	templateUrl: './directorship-test.component.html',
	styleUrls: ['./directorship-test.component.scss'],
})
export class DirectorshipTestComponent implements OnInit {
	userData: any = {};
	errorMessages = businessDocumentsFieldForm;
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.DIRECTORSHIP_TEST),
		...
		this.verificationDetailsService.list(params).subscribe({
			next: (result) => {
				...
				if (result.data && result.data.length > 0) {
					...
					this.directorship = result.data[0];
	...
	onSubmit(): void {
		...
		this.verificationMediaService.createBulk(this.dataForm.getRawValue()).subscribe({
			next: (result) => {
				this.spinnerService.stop();
				this.directorship = result.data;
				this.dataForm.patchValue(result.data);
				this.toastService.success(result.message);
				this.verificationDetailsService.showCompleteModal('directorship-test', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	patch(payload) {
		this.verificationDetailsService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('directorship-test', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'directorship-test', this.userData);
	}
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.DIRECTORSHIP_TEST)
		);
	}
}
