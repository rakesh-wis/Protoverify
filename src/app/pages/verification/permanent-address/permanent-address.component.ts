...
export class PermanentAddressComponent implements OnInit {
	...
	dataForm = new FormGroup({
		...
		permanentVerificationAddress: new FormGroup({
	...
	get permanentVerificationAddress() {
		return this.dataForm.controls['permanentVerificationAddress']['controls'];
	}
	...
	searchPincode() {
		...
		if (this.permanentVerificationAddress['pincode'].value.toString().length < 6) {
			return;
		}
		this.sharedService.searchPincode(this.permanentVerificationAddress['pincode'].value).subscribe(
			(result) => {
				if (result.PostOffice) {
					this.permanentVerificationAddress['city'].setValue(result.PostOffice[0].District);
					this.permanentVerificationAddress['state'].setValue(result.PostOffice[0].State);
	...
	onSubmit(): void {
		...
		this.verificationMediaService.createBulk(this.dataForm.getRawValue()).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.showCompleteModal('permanent-address', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	openConfirmStatus(status) {
		if (status === verificationLevel.CLEAR_REPORT) {
	...
	patch(payload) {
		this.verificationDetailsService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('permanent-address', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'permanent-address', this.userData);
	}
	...
	loadVendorData(filters = {}): void {
		let params = {
			start: (this.page - 1) * this.pageVendor,
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.PERMANENT_ADDRESS)
		);
	}
}
