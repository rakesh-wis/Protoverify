...
@Component({
	selector: 'app-police-verification-police',
	templateUrl: './police-verification-police.component.html',
	styleUrls: ['./police-verification-police.component.scss'],
})
export class PoliceVerificationPoliceComponent implements OnInit {
	businessDocuments = [
		{
			label: 'Police verification Certificate',
			mediaType: 'police_verification_through_police',
		},
	];
	userData: any = {};
	dataForm = new FormGroup({
		id: new FormControl(),
		type: new FormControl(planFeatures.POLICE_VERIFICATION_THROUGH_POLICE),
	...
	createItem(item: any): FormGroup {
		return this.formBuilder.group({
			...
			mediaType: new FormControl('police_verification_through_police'),
		});
	}
	...
	onSubmit(): void {
		...
		this.verificationMediaService.createBulk(this.dataForm.getRawValue()).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.showCompleteModal('police-verification', 'pending', 1);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	patch(payload) {
		this.verificationDetailsService.patch(payload).subscribe({
			next: (result) => {
				...
				this.verificationDetailsService.updateCheckList('police-verification', this.form['status'].value);
			},
			error: (error) => {
				this.spinnerService.stop();
				this.toastService.error(error);
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'police-verification', this.userData);
	}
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.POLICE_VERIFICATION_THROUGH_POLICE)
		);
	}
}
