...
export class PostalAddressComponent implements OnInit {
	...
	createItem(item: any): FormGroup {
		return this.formBuilder.group({
			...
			status: new FormControl(kycStatus.PENDING),
	...
	addItems(): void {
		...
		Object.values([
			employeeAddressType.POSTAL_PERMANENT_ADDRESS,
			employeeAddressType.POSTAL_CURRENT_ADDRESS,
		]).forEach((element) => {
				...
				if (data.data.count != 0) {
					this.addresses.controls[0].patchValue(
						![ROLES.EMPLOYEE].includes(this.currentUser.role)
							? data.data['rows'].find(
									(element) => element.type === employeeAddressType.POSTAL_PERMANENT_ADDRESS
							  )
							: data.data.find((element) => element.type === employeeAddressType.POSTAL_PERMANENT_ADDRESS)
					);
					...
					if (![ROLES.EMPLOYEE].includes(this.currentUser.role)) {
						if (
							data.data['rows'].find(
								(element) => element.type === employeeAddressType.POSTAL_CURRENT_ADDRESS
							)
						) {
						...
						if (data.data.find((element) => element.type === employeeAddressType.POSTAL_CURRENT_ADDRESS)) {
							this.addresses.controls[1].patchValue(
								data.data.find((element) => element.type === employeeAddressType.POSTAL_CURRENT_ADDRESS)
							);
							if (this.addresses.controls[1].get('isSameAsRegistered').value) {
								this.addresses.controls[1].disable();
								this.addresses.controls[1].get('isSameAsRegistered').enable();
					...
					if ([verificationLevel.CLEAR_REPORT].includes(this.addresses.controls[1].get('status').value)) {
	...
	goNext() {
		...
		this.verificationDetailsService.navigateToNext(path, 'postal-address', this.userData);
	}
	verifyOtp() {
		...
		this.addressService
			...
			.subscribe({
				next: (result) => {
					...
					this.toastService.success(result.message);
					this.dataForm.disable();
					this.verificationDetailsService.updateCheckList('postal-address', this.form['status'].value);
				},
				error: (error) => {
					this.spinnerService.stop();
					this.toastService.error(error);
	...
	patchAadharAddress(isPermanent: boolean) {
		this.spinnerService.start();
		this.kycService.getAadharAddress({ userId: this.userId }).subscribe({
			next: (result) => {
				const address = result.data;
				this.spinnerService.stop();
				if (isPermanent) {
					this.addresses.controls[0].get('addressLine1').setValue(address.house + ', ' + address.loc);
					this.addresses.controls[0].get('addressLine2').setValue(address.landmark + ', ' + address.street);
					this.addresses.controls[0].get('pincode').setValue(address.pincode);
					this.searchPincode(0);
					this.showSameAsAadharButton = false;
				} else {
					this.addresses.controls[1].get('addressLine1').setValue(address.house + ', ' + address.loc);
					this.addresses.controls[1].get('addressLine2').setValue(address.landmark + ', ' + address.street);
					this.addresses.controls[1].get('pincode').setValue(address.pincode);
					this.searchPincode(1);
					this.showSameAsAadharButton = false;
				}
			},
			error: (error) => {
	...
	areInputsDisabled(): boolean {
		return (
			...
			!this.assignedVerificationChecks.includes(this.planFeatures.POSTAL_ADDRESS)
		);
	}
}
