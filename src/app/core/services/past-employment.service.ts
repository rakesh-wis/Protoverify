...
export class PastEmploymentService {
	...
	delete(id: any): Observable<any> {
		const url = `/admin/employee/past-organization/${id}`;
	...
	buildStatus(payload: any): Observable<any> {
		const url = `/admin/employee/past-organization/bulk-status`;
	...
	updateEmailAttachment(payload: any): Observable<any> {
		const url = `/admin/employee/past-organization/attachments/${payload.id}`;
		return this.apiService.patch(url, payload, SERVICES.USER).pipe(
			map((data) => {
				if (data && data.result) {
					return data.result;
	...
	updateHrResponse(url: string, payload: any): Observable<any> {
		return this.apiService.patch(url, payload, SERVICES.USER).pipe(
			map((data) => {
				if (data && data.result) {
					return data.result;
